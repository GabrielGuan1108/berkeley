...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(_) for _ in clusters]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    Sxy = sum([(xs[_] - mean(xs))*(ys[_] - mean(ys)) for _ in range(0,len(xs)) ])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    a = [find_predictor(user, reviewed, _) for _ in feature_fns]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for _ in restaurants:
        xxx = restaurant_name(_)
        if _ in reviewed:
            r_v[xxx] = user_rating(user, xxx)
        else:
            r_v[xxx] = predictor(_)
    return r_v
...
def search(query, restaurants):
    ...
    """
    # BEGIN Question 10
    return [b for b in restaurants if query in restaurant_categories(b)]
    "*** YOUR CODE HERE ***"
    # END Question 10
