...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        """
        if insect.is_ant:
            if self.ant is None: #没有蚂蚁，放一个进去
                self.ant = insect
            else:
                # BEGIN Problem 9
                #print(self.ant.is_container, self.ant.can_contain(insect))
                if self.ant.is_container and self.ant.can_contain(insect):#有蚂蚁，但是它是空的container
                    #print("I'm here")
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt):#这里简单粗暴 不过doctest好用
                if insect.count == 0:
                    return None
...
class Bee(Insect):
    ...
    def action(self, colony, direction = 1):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        count = 0
        while (place is not hive) and (place is not None):
            if (len(place.bees) != 0) and (count <= self.max_range) and (count >= self.min_range):
...
class ShortThrower(ThrowerAnt):
    ...
    implemented = False
    food_cost = 2
    #damage = 1# Change to True to view in the GUI
    # END Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            ...
            copy_b = bees.copy()
            bees = [a.reduce_armor(self.damage) for a in copy_b]
            #for _ in range(0, len(copy_b)):
                #bees[_].reduce_armor(self.damage)
            self.place.remove_insect(self)
...
class BodyguardAnt(Ant):
    ...
    def __init__(self, armor=2):
        ...
        #self.damage = 0  # The Ant hidden in this bodyguard
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        def container_d(ant):
            ...
            print('DEBUG:',c_ant)
            if c_ant and (c_ant not in self.changed):
                print('DEBUG:',c_ant.damage)
                self.changed.append(c_ant)
                c_ant.damage *= 2
                print('DEBUG:',c_ant.damage)
        p = self.place.exit#第一个要从Queen的前一格开始加damage
        if self.count != 0: #如果不是第一个Queen要自杀
            self.reduce_armor(self.armor)
        else:
            while p is not None:#p is not None指的是还有格子，没有loop到场地外
                ant = p.ant
                if ant :
                    if ant not in self.changed:
                        print('DEBUG:',ant.damage)
                        self.changed.append(ant)
                        ant.damage *= 2
                        print('DEBUG:',ant.damage)
