...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and isinstance(env.lookup(first), MacroProcedure):
        return scheme_eval(env.lookup(first).apply_macro(rest, env), env, tail=True)
    else:
        # BEGIN PROBLEM 5
        procedure = scheme_eval(first, env)
        left = rest.map(lambda operand: scheme_eval(operand, env))
        return scheme_apply(procedure, left, env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        lambda_procedure = LambdaProcedure(target.second, expressions.second, env)
        first = target.first
        env.define(first, lambda_procedure)
        return first
        # END PROBLEM 10
    else:
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 21
    check_form(expressions, 2, 2)
    target, body = expressions.first, expressions.second
    check_form(target, 1)
    name, formal = target.first, target.second
    if scheme_symbolp(name):
        macro = MacroProcedure(formal, body, env)
        env.define(name, macro)
        return name

    bad_name = name.first if isinstance(name, Pair) else name
    raise SchemeError('non-symbol: {0}'.format(bad_name))
    # END Problem 21
